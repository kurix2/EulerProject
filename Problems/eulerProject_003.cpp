...
void problem3(long long a) {
  ...
  // I did this to increase efficiency if possible
  while (a%2 == 0) {
    std::cout << 2;
    a = a/2;
  }

  // i has to be an odd number, so I started at 3 and iterated by 2
  // so that the i value is never an even number
  for (int i = 3; i <= sqrt(a); i = i+2) {
    // While i is divisible by a, print it and divide it by a
    while (a%i == 0) {
      std::cout << i << std::endl;
      a /= i;
    }
  ...
  // If a is a prime umber that is greater than 2 then print it
